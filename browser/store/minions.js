...
export const createMinionThunk = minion => dispatch => {
  axios.post('http://localhost:4002/api/minions', minion)
  .then(res => res.data)
  .then(createdMinion => {
...
export const updateMinionThunk = minion => dispatch => {
  axios.put(`http://localhost:4002/api/minions/${minion.id}`, minion)
  .then(res => res.data)
  .then(updatedMinion => {
...
export const deleteMinionThunk = minionId => dispatch => {
  axios.delete(`http://localhost:4002/api/minions/${minionId}`)
  .then(res => res.data)
  .then(() => {
    return axios.get(`http://localhost:4002/api/minions`)
  })
  .then(res => res.data)
  .then(allMinions => {
