...
const appEnter = nextRouterState => {
  Promise.all([
    axios.get('http://localhost:4002/api/minions'),
    axios.get('http://localhost:4002/api/ideas'),
    axios.get('http://localhost:4002/api/meetings'),
  ])
...
const singleMinionEnter = nextRouterState => {
  ...
  axios.get(`http://localhost:4002/api/minions/${id}`)
  .then(res => res.data)
  .then(minion => {
  ...
  axios.get(`http://localhost:4002/api/minions/${id}/work`)
  .then(res => res.data)
  .then(work => {
...
const singleIdeaEnter = nextRouterState => {
  const id = nextRouterState.params.id;
  axios.get(`http://localhost:4002/api/ideas/${id}`)
  .then(res => res.data)
  .then(idea => {
